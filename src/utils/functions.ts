...
export const isPauseBackup = (start: string, end: string) => {
    const startTime = new Date(start);
    const endTime = new Date(end);
    const currentTime = new Date();
    const startTimeInRange = new Date().setHours(
        startTime.getHours(),
        startTime.getMinutes(),
        startTime.getSeconds(),
    );
    const endTimeInRange = new Date().setHours(
        endTime.getHours(),
        endTime.getMinutes(),
        endTime.getSeconds(),
    );
    ...
    return (
        currentTime.getTime() >= startTimeInRange &&
        currentTime.getTime() <= endTimeInRange
    );
};